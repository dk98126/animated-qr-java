...
public class WaveFile {
    ...
    public void createAudioFile(String fileName) throws Exception
    {
        FileInputStream fin=new FileInputStream(fileName);
        byte[] bytesoffile=new byte[fin.available()];
        fin.read(bytesoffile,0,bytesoffile.length);
        int[] samples=new int[bytesoffile.length];
        for(int i=0; i<bytesoffile.length;i++)
        samples[i]= (int)Math.round((Integer.MAX_VALUE/2)*(Math.sin(2*Math.PI*440*bytesoffile[i]*i/44100)));
        WaveFile wf = new WaveFile(4, 3000, 1, samples);
